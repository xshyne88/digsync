defmodule DigsyncWeb.Live.Components.DatePickerComponent do
  ...
  def render(assigns) do
    ...
    <div class="date_picker" x-data="{ calendar_open : @calendar_open }">
      <h1>
        Showing games on:
        <button @click="{ calendar_open ? calendar_open = false : calendar_open = true }">
      ...
      <div class="calendar" x-show="calendar_open" @click.away="calendar_open = false">
        <button
          ...
          phx-value-month="{@calendar.previous_month}"
          phx-target="{@myself}"
        >
          <svg width="5" height="8" viewBox="0 0 5 8" fill="none" xmlns="http://www.w3.org/2000/svg">
            <path d="M4 7L1 4L4 1" stroke="#829AB1" stroke-width="1" />
          </svg>
        </button>
        ...
        <button
          class="next_month"
          phx-click="change_month"
          phx-value-month="{@calendar.next_month}"
          phx-target="{@myself}"
        >
          <svg width="5" height="8" viewBox="0 0 5 8" fill="none" xmlns="http://www.w3.org/2000/svg">
            <path d="M1 1L4 4L1 7" stroke="#829AB1" stroke-width="1" />
          </svg>
        </button>
        <ul class="dow">
          <li>S</li>
        ...
        <%= for week <- @calendar.days_by_week do %>
          <ul class="week">
            <%= for date <- week do %>
              <%= print_calendar_date(
                %{date: date, selected_day: @calendar.selected_day, target: @myself},
                assigns
              ) %>
            <% end %>
          </ul>
  ...
  def update(assigns, socket) do
    socket =
      assign(socket, :calendar, calendar_info(assigns.selected_date, assigns.selected_date))
